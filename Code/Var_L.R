...
## Code to create the plot of delay time varying the window size, Figure 8 in the main paper
...
L_grid = c(10,20,30,40,50,60,70,80)   ## window length for KS
k_alpha = 3 ## KS
  ...
  for(ind_T in 1:length(T_grid))
  {
    ...
    for(ind_d in 1:length(d_grid))
    {
      ...
      for(ind_TRF in 1:length(Total_rate_factor_grid))
      {
        ...
        ################################################################      
        ## KS tres_hold
        NN =  100  # number of MC simulations to estimate threshold
